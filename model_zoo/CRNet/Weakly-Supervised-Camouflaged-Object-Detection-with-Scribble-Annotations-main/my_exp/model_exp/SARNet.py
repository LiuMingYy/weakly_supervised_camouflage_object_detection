...
class SARNet(nn.Module):
    def __init__(self, fun_str = 'pvt_v2_b0'):
        ...
        self.cab =CAB(in_feat=embedding_dims[3], out_feat=embedding_dims[3] // 8,
                      kernel_size=3, reduction=4, bias=False, act=nn.PReLU())
    ...
    def forward(self, x):
        ...
        # s5 = self.cbr(layer[3])  # 1,64,21,21
        s5 = self.cab(layer[3])
...
class CAB(nn.Module):
    def __init__(self, in_feat, out_feat, kernel_size, reduction, bias, act):
        ...
        self.CA = CALayer(out_feat, reduction, bias=bias)

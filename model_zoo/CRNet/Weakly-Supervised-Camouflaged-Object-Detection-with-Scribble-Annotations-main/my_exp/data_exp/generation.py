...
def get_pixel_similarity(image, position, window_size=15):
    """
    计算指定位置的像素点与相邻像素的相似性
    :param image: 输入图像，可以是 PIL 图像或者 PyTorch 张量
    :param position: 像素位置，一个包含 x 和 y 坐标的元组或列表
    :param window_size: 局部窗口大小，一个整数或者元组，表示窗口的宽度和高度
    :return: 相似性的度量值，一个标量 Tensor
    """
    # 将输入图像转换为 Lab 颜色空间
    ...
    for i in range(x - w // 2, x + w // 2 + 1):
        for j in range(y - h // 2, y + h // 2 + 1):
            # 如果像素位置超出图像边界，则跳过该像素点
            if i < 0 or i >= image.shape[0] or j < 0 or j >= image.shape[1]:
                continue
            # 计算当前像素点的 Lab 颜色值与指定位置像素点的 Lab 颜色值之间的欧几里得距离
            q = image[i, j, :]
            distance = torch.sqrt(torch.sum((pixel - q) ** 2))
            distances.append(distance.item())

    # 计算列表中的最小方差，作为相似性的度量值
    variance = torch.var(torch.tensor(distances))
    return variance


def get_position_similarity(image=None, b=[], f=[]):
    variance_list = []
    for i in range(len(b)):
        b_position = b[i]
        var = get_pixel_similarity(image, b_position, window_size=15)
...
def select_point(res, img):
    ...
    # select point
        ...
        get_position_similarity(image=img, b=bg, f=fg)
...
if __name__ == '__main__':
    savePath = '/mnt/disk/lym/COD10K/TrainDataset/myexp/'
    trainPath = '/mnt/disk/lym/COD10K/TrainDataset/Imgs'
    scribblePath = '/mnt/disk/lym/COD10K/TrainDataset/Scribble'
    i=0
    ...
    txt_path = '/mnt/disk/lym/COD10K/TrainDataset/myexp/bad_data_name.txt'

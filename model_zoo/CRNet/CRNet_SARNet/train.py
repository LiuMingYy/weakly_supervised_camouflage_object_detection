...
EXP_NAME = f'CRNet_exp_SARNet'
pvt_name = 'pvt_v2_b3'
savepath = f'/mnt/disk/lym/COD10K/result/COD10K/{EXP_NAME}/'
if not os.path.exists(savepath):
    os.makedirs(savepath)
TAG = "scribblecod"
log_path = os.path.join(savepath, 'log.log')

logging.basicConfig(level=logging.INFO, format='%(levelname)s %(asctime)s %(filename)s: %(lineno)d] %(message)s',
                   datefmt='%Y-%m-%d %H:%M:%S',
                   filename=savepath + 'log.log', filemode="a")
...
def train(Dataset, Network, cfg, train_loss, start_from = 0):
    ...
    # val_cfg = [Dataset.Config(datapath=f'{root}/TestDataset/{i}' , mode='test')
    ...
    ## log
    sw = SummaryWriter(cfg.savepath + 'summary')
    db_size = len(loader)
...
if __name__=='__main__':
    ...
    cfg = dataset.Config(datapath=f'{root}', savepath=savepath, mode='train', batch=12, lr=1e-3, momen=0.9, decay=5e-4, epoch=total_epoch, label_dir = 'Scribble')
    # from net import Net
    from my_exp.model_exp.SARNet import SARNet
    Net = SARNet(pvt_name)

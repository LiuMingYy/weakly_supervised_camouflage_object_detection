...
class ResNet(nn.Module):
    ...
    def initialize(self):
        self.load_state_dict(torch.load('/mnt/disk/lym/pth/resnet50-19c8e357.pth'), strict=False)
...
class CoordAtt(nn.Module):
    ...
    def forward(self, x):  # x:16,32,40,40
        identity = x
        
        n,c,h,w = x.size()
        x_h = self.pool_h(x)  # 16,32,40,1
        x_w = self.pool_w(x).permute(0, 1, 3, 2)  # 16,32,40,1

        y = torch.cat([x_h, x_w], dim=2)  # 16,32,80,1
        y = self.conv1(y)  # 16,8,80,1
        y = self.bn1(y)
        y = self.act(y) 
        
        x_h, x_w = torch.split(y, [h, w], dim=2)  # 16,8,40,1
        x_w = x_w.permute(0, 1, 3, 2)  # 16,8,1,40

        a_h = self.conv_h(x_h).sigmoid()  # 16,32,40,1
        a_w = self.conv_w(x_w).sigmoid()  # 16,32,1,40

...
EXP_NAME = f'SARNet_insert_CAMOv4'
pvt_name = 'pvt_v2_b3'
...
logging.basicConfig(level=logging.INFO, format='%(levelname)s %(asctime)s %(filename)s: %(lineno)d] %(message)s',
                   datefmt='%Y-%m-%d %H:%M:%S',
                   filename=savepath + 'save_log/' + 'log.log', filemode="w")
...
def train(Dataset, Network, cfg, train_loss, start_from = 0):
    ...
    for epoch in range(start_from, cfg.epoch):
        ...
        batch_idx = -1
        image, image1, mask, insert_mask, mask1 = prefetcher.next()  # mask1 合成区域
...
if __name__=='__main__':
    ...
    cfg = dataset.Config(datapath=f'{root}', savepath=savepath, mode='train', batch=5, lr=1e-3, momen=0.9, decay=5e-4, epoch=total_epoch, label_dir = 'Scribble')
    # from net import Net

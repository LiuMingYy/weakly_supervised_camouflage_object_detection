...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias, opr_kernel_size=3,iterations=1):
    ...
    def forward(self, x, mask=None):
        ...
        q=self.qkv_0(x)  # 1,64,24,24
        k=self.qkv_1(x)  # 1,64,24,24
        v=self.qkv_2(x)  # 1,64,24,24
        ...
        q=self.qkv1conv(q)  # 1, 64, 24, 24
        k=self.qkv2conv(k)  # 1, 64, 24, 24
        v=self.qkv3conv(v)  # 1, 64, 24, 24

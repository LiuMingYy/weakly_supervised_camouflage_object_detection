...
class FeatureLoss(torch.nn.Module):
    ...
    def forward(
        ...
        N, C, height_pred, width_pred = y_hat_softmax.shape  # N:16 C:1 height_pred:80 width_pred:80
        ...
        y_hat_unfolded = self._unfold(y_hat_softmax, kernels_radius)  # 16,1,11,11,80,80
        # 计算每个像素周围半径为kernels_radius=5的像素之间的差值
    ...
    def _create_kernels(
        ...
        for i, desc in enumerate(kernels_desc):
            weight = desc['weight']  # weight:1
            features = []
            for modality, sigma in desc.items():
                if modality == 'weight':
                    continue
                if modality == 'xy':
                    feature = FeatureLoss._get_mesh(N, height_pred, width_pred, device)  # 16,2,80,80
                else:
            ...
            features = torch.cat(features, dim=1)  # 16,5,80,80
            kernel = weight * FeatureLoss._create_kernels_from_features(features, kernels_radius)  # 16,1,11,11,80,80
    ...
    def _create_kernels_from_features(features, radius):
        ...
        N, C, H, W = features.shape  # N:16, C:5, H:80, W:80
        kernels = FeatureLoss._unfold(features, radius)  # 16,5,11,11,80,80
        kernels = kernels - kernels[:, :, radius, radius, :, :].view(N, C, 1, 1, H, W)  # 将中心像素设置为0
        kernels = (-0.5 * kernels ** 2).sum(dim=1, keepdim=True).exp()  # K_vis(i,j)  16,1,11,11,80,80
        # kernels[:, :, radius, radius, :, :] = 0
        return kernels

    @staticmethod
    def _get_mesh(N, H, W, device):  # 生成二维坐标网格，用于计算每个像素与其周围像素的距离
        return torch.cat((
    ...
    def _unfold(img, radius):
        ...
        N, C, H, W = img.shape  # N:16,C:1,H:80,W:80
        diameter = 2 * radius + 1  # radius:5 直径

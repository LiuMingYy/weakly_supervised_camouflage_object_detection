...
def train(Dataset, Network, cfg, train_loss, start_from = 0):
    ...
    # val_cfg = [Dataset.Config(datapath=f'{root}/TestDataset/{i}' , mode='test') for i in ['CHAMELEON', 'CAMO', 'COD10K']]
    ...
    for epoch in range(start_from, cfg.epoch):
        ...
        batch_idx = -1
        image, mask, syn_image, syn_mask, region = prefetcher.next()
        while image is not None:
            ...
            loss2, loss3, loss4, loss5, loss6 = train_loss(image, mask, syn_image, syn_mask, region, net, dict(epoch=epoch+1, global_step=global_step, sw=sw, t_epo=cfg.epoch))

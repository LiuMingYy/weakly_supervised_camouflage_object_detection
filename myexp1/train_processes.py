...
def train_loss(image, mask, syn_image, syn_mask, region, net, ctx, ft_dct, w_ft=.1, ft_st = 60, ft_fct=.5, ft_head=True, mtrsf_prob=1, ops=[0,1,2], w_l2g=0, l_me=0.1, me_st=50, me_all=False, multi_sc=0, l=0.3, sl=1, sb=28, alpha_syn=1):
    """
    w_ft:0.15
    ft_st:60
    ft_fct:0.5
    ...
    if epoch >= sb:  # sb=28
        ...
        prediction *= 255
        # count_255p = np.sum(prediction > 127)
        # count_0p = np.sum(prediction <= 127)

        region = region.data.cpu().numpy()
        # region = (region - region.min()) / (region.max() - region.min() + 1e-8)
        # region *= 255
        # count_255 = np.sum(region > 127)
        # count_0 = np.sum(region <= 127)

        # 将图像二值化
        removed_prediction = np.zeros_like(prediction)
        removed_prediction[prediction>127] = 255

        # unique_values1 = np.unique(removed_prediction)
        # is_binary1 = len(unique_values1) == 2
        # print('Is binary image:', is_binary1)
        ...
        # count_255pp = np.sum(prediction > 127)
        # count_0pp = np.sum(prediction <= 127)
        removed_prediction = torch.from_numpy(removed_prediction).cuda()

        syn_out2, _, _, _, _, _ = net(syn_image, )
        syn_prediction = syn_out2.sigmoid()
        syn_prediction = torch.cat((1 - syn_prediction, syn_prediction), 1)

        # Local Saliency Coherence Loss (Context Affinity Loss)
        syn_image_ = F.interpolate(syn_image, scale_factor=0.25, mode='bilinear', align_corners=True)  # 0.25下采样
        syn_sample = {'rgb': syn_image_}
        syn_out2_ = F.interpolate(syn_prediction[:, 1:2], scale_factor=0.25, mode='bilinear', align_corners=True)
        ...
        removed_prediction_s = F.interpolate(removed_prediction, scale_factor=0.25, mode='bilinear', align_corners=True)
        loss_ssc_syn = SelfConsistency(syn_out2_, removed_prediction_s.detach(), alpha=0.5)
        ...
        loss_syn = loss_lsc_syn + loss_ssc_syn + criterion(syn_prediction, syn_bg_label) + criterion(syn_prediction, syn_fg_label)
    else:
        loss_syn = 0
    ...
    loss2 = loss_ssc + criterion(out2, fg_label) + criterion(out2, bg_label) + l * loss2_lsc + loss_intra[0] + alpha_syn*loss_syn  ## dominant loss
